...
public class HealthStatus {
    ...
    public void getStatus() throws SQLException {
        String query = "SELECT * FROM healthstatus where Acc_id = " + "'" + this.userID + "'";
        Statement stmt = con.createStatement();
        ResultSet rs = stmt.executeQuery(query);

        if (rs.next()) {
            String a = rs.getString("Acc_ID");
            this.weight = rs.getInt("P_Weight");
            this.height = rs.getInt("P_height");
            this.bloodPressure = rs.getString("P_bloodpressure");

            this.bloodSugar = rs.getString("P_bloodsugar");
            this.unineSuger = rs.getString("P_urinesugar");
